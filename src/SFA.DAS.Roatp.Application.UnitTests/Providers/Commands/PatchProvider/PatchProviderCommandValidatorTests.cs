...
public class PatchProviderCommandValidatorTests
{
    ...
    [TestCase(":", false)]
    [TestCase("=", false)]
    public async Task Validate_MarketingInfo_AllowSelectedSpecialCharacters(string info, bool isValid)
    {
        var sut = new PatchProviderCommandValidator(_providersReadRepo.Object);
        var ukprn = 10000001;
        ...
        command.Patch = new JsonPatchDocument<Domain.Models.PatchProvider>
        {
            ...
            {
                new Operation<Domain.Models.PatchProvider>
                    { op = Replace, path = MarketingInfo, value = info },
            }
        };
        ...
        {
            result.Errors.Should().HaveCount(1);
            result.Errors[0].ErrorMessage.Contains(ValidationMessages.InvalidCharactersErrorMessage).Should().BeTrue();
        }
    }
