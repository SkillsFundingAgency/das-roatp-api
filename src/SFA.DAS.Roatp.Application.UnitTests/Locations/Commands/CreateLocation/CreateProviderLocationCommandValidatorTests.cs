...
public class CreateProviderLocationCommandValidatorTests
{
    ...
    [TestCase("location - name.", true)]
    [TestCase("<location name>", false)]
    public async Task Validate_LocationName_HasAllowedCharacters(string locationName, bool isValid)
    {
        var validator = GetDefaultValidator();
        var result = await validator.TestValidateAsync(new CreateProviderLocationCommand { LocationName = locationName });
        if (isValid)
            ...
            result.ShouldHaveValidationErrorFor(c => c.LocationName).WithErrorMessage(ValidationMessages.InvalidCharactersErrorMessage);
    }
    ...
    [TestCase("=", false)]
    [TestCase("{", false)]
    public async Task Validate_CharacterValidations(string value, bool isValid)
    {
        var validator = GetDefaultValidator();
        var result = await validator.TestValidateAsync(new CreateProviderLocationCommand
        {
            LocationName = value,
            AddressLine1 = value,
            AddressLine2 = value,
        ...
        {
            result.ShouldHaveValidationErrorFor(c => c.LocationName).WithErrorMessage(ValidationMessages.InvalidCharactersErrorMessage);
            result.ShouldHaveValidationErrorFor(c => c.AddressLine1).WithErrorMessage(ValidationMessages.InvalidCharactersErrorMessage);
            result.ShouldHaveValidationErrorFor(c => c.AddressLine2).WithErrorMessage(ValidationMessages.InvalidCharactersErrorMessage);
            result.ShouldHaveValidationErrorFor(c => c.Town).WithErrorMessage(ValidationMessages.InvalidCharactersErrorMessage);
            result.ShouldHaveValidationErrorFor(c => c.Postcode).WithErrorMessage(ValidationMessages.InvalidCharactersErrorMessage);
            result.ShouldHaveValidationErrorFor(c => c.County).WithErrorMessage(ValidationMessages.InvalidCharactersErrorMessage);
        }
    }

    private static CreateProviderLocationCommandValidator GetDefaultValidator()
    {
        Mock<IProviderLocationsReadRepository> locationRepoMock = new();
        locationRepoMock.Setup(r => r.GetAllProviderLocations(It.IsAny<int>())).ReturnsAsync([]);
        return new CreateProviderLocationCommandValidator(Mock.Of<IProvidersReadRepository>(), locationRepoMock.Object);
    }
}
