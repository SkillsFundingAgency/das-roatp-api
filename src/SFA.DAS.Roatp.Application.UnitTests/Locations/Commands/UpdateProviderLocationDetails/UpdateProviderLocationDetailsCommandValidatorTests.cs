...
public class UpdateProviderLocationDetailsCommandValidatorTests
{
    ...
    [TestCase("=", false)]
    [TestCase("{", false)]
    public async Task Validate_LocationNameCharacterValidations(string locationName, bool isValid)
    {
        var validator = GetDefaultValidator();
        var result = await validator.TestValidateAsync(new UpdateProviderLocationDetailsCommand { LocationName = locationName });
        if (isValid)
    ...
    private static UpdateProviderLocationDetailsCommandValidator GetDefaultValidator()
    {
        Mock<IProviderLocationsReadRepository> locationRepoMock = new();
        locationRepoMock.Setup(r => r.GetAllProviderLocations(It.IsAny<int>())).ReturnsAsync(new List<ProviderLocation>());
        return new UpdateProviderLocationDetailsCommandValidator(Mock.Of<IProvidersReadRepository>(), locationRepoMock.Object);
    }
}
