...
public class CreateProviderCourseCommandValidatorContactDetailsValidationTests : CreateProviderCourseCommandValidatorTestBase
{
    ...
    [TestCase(":", false)]
    [TestCase("=", false)]
    public async Task Phone_Character_Validation(string phone, bool isValid)
    {
        var command = new CreateProviderCourseCommand { ContactUsPhoneNumber = phone };
        var sut = GetSut();
        ...
        if (isValid)
            ...
            result.ShouldHaveValidationErrorFor(c => c.ContactUsPhoneNumber).WithErrorMessage(ValidationMessages.InvalidCharactersErrorMessage);
    }
