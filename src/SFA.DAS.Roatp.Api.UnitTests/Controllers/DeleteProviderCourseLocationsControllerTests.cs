...
namespace SFA.DAS.Roatp.Api.UnitTests.Controllers
{
    ...
    public class DeleteProviderCourseLocationsControllerTests
    {
        ...
        public async Task BulkDeleteProviderCourseLocations_CallsHandler(
            ...
            int ukprn, int larsCode, DeleteOptions options)
        {
            await sut.BulkDeleteProviderCourseLocations(ukprn, larsCode, options);

            _mediatorMock.Verify(m => m.Send(It.Is<BulkDeleteProviderCourseLocationsCommand>(c => c.Ukprn == ukprn && c.LarsCode == larsCode && c.DeleteOptions == options), It.IsAny<CancellationToken>()));
        }
        ...
        public async Task BulkDeleteProviderCourseLocations_ZeroResults_ReturnsNotFoundResponse(
            ...
            int ukprn, int larsCode, DeleteOptions options)
        {
            _mediatorMock.Setup(m => m.Send(It.Is<BulkDeleteProviderCourseLocationsCommand>(c => c.Ukprn == ukprn && c.LarsCode == larsCode && c.DeleteOptions == options), It.IsAny<CancellationToken>())).ReturnsAsync(0);
        ...
        public async Task BulkDeleteProviderCourseLocations_MoreThanZeroResults_ReturnsNoContentResponse(
            ...
            int ukprn, int larsCode, DeleteOptions options)
        {
            _mediatorMock.Setup(m => m.Send(It.Is<BulkDeleteProviderCourseLocationsCommand>(c => c.Ukprn == ukprn && c.LarsCode == larsCode && c.DeleteOptions == options), It.IsAny<CancellationToken>())).ReturnsAsync(1);
