...
namespace SFA.DAS.Roatp.Api.UnitTests.Controllers
{
    ...
    public class ProviderLocationsBulkInsertControllerTests
    {
        ...
        [Test, MoqAutoData]
        public async Task ProviderLocationsBulkInsert_ZeroResults_ReturnsNoContentResponse(
            [Frozen] Mock<IMediator> _mediatorMock,
            [Greedy] ProviderLocationsBulkInsertController sut,
            int ukprn, ProviderLocationsInsertModel providerLocationsInsertModel)
        {
            _mediatorMock.Setup(m => m.Send(It.Is<BulkInsertProviderLocationsCommand>(c => c.Ukprn == ukprn && c.LarsCode == providerLocationsInsertModel.LarsCode), It.IsAny<CancellationToken>())).ReturnsAsync(0);
